...
export function getScreentimeAlertList (users, date){
    ...
    users.map( user => {
    let count = 0 ;
    user.screenTime.map( x => {
      if (x.date === date) {
        for (let key in x.usage) {
          count += x.usage[key];
          }
        }
      } );
...
export function findWinner (board) {
    ...
    // check for rows 
    if (arr[0] === arr[1] && arr[0] === arr[2] && arr[0] !== "") return arr[0];
    if (arr[3] === arr[4] && arr[3] === arr[5] && arr[3] !== "") return arr[3];
    if (arr[6] === arr[7] && arr[6] === arr[8] && arr[6] !== "") return arr[6];
  
    // check for columns 
    if (arr[0] === arr[3] && arr[0] === arr[6] && arr[0] !== "") return arr[0];
    if (arr[1] === arr[4] && arr[1] === arr[7] && arr[1] !== "") return arr[1];
    if (arr[2] === arr[5] && arr[2] === arr[8] && arr[2] !== "") return arr[2];
  
    // check for diagonals
    if (arr[0] === arr[4] && arr[0] === arr[8] && arr[0] !== "") return arr[0];
    if (arr[2] === arr[4] && arr[2] === arr[6] && arr[2] !== "") return arr[2];
